...
parquet_filename = 'Degradation_exp2_peaks.parquet'
...
for i in range(len(degr_tests)):
    ...
    ignore_ind_list = Protocol_objects.Acoustic_p2C.ignore_ind_list
    #---------------------------------
    filter = ~df_sub.index.isin(ignore_ind_list)
    x1 = df_sub['cycling'].loc[filter, 'Time(s)'].to_numpy()/3600
    x2 = df_sub['cycling']['Time(s)'].to_numpy()/3600
    volts = df_sub['cycling'].loc[filter, 'V(V)']
    # Voltage vs time
...
save_filename = 'Exp2_full_protocols'
...
def make_grids(Pulse_direction = 'dischrg', Peak = -1):
    ...
    if Pulse_direction == 'dischrg':
        pulse_range = range(0, 15)
    elif Pulse_direction == 'chrg':
        pulse_range = range(0, 3) 
    #
    grids = {
...
socs_dischrg = [100, 95, 90, 85, 80, 75, 70, 65, 60, 55, 50, 45, 40, 35, 30]
socs_chrg = [25, 30, 35]
ocvs_pre_mean_dischrg = [
...
ocvs_pre_mean_chrg = [3.6973565, 3.725144, 3.76211]
...
def plot_degr_pulses(color_by = 'cycle', y_quant = 'tof',
    ...
    if Pulse_direction == 'dischrg':
        ...
        figsize=(8,7)
        socs = socs_dischrg
        ocvs_pre_mean = ocvs_pre_mean_dischrg
    elif Pulse_direction == 'chrg':
        pulse_range = range(0, 3)
        grids = grids_chrg
        nrows, ncols = 1, 3
        figsize=(6,2.5)
        socs = socs_chrg
        ocvs_pre_mean = ocvs_pre_mean_chrg
    ...
    for count, Pulse_id in enumerate(pulse_range):
        ...
        axs_row = count//(4+1)
        axs_column = count%(4+1)
        if nrows == 1:
            ax = axs[axs_column]
        else:
            ax = axs[axs_row, axs_column]
        for i in range(len(degr_tests)):
            label = cycles_and_days[i]
        ...
        title = 'Pulse {}\n(~{:.0f}%, ~{:.2f}V)'.format(Pulse_id+1, socs[Pulse_id], ocvs_pre_mean[Pulse_id])
        ax.set_title(title)
    ...
    if time_window == 'short':
        ax.set_xlim([0, 13])
        ax.set_xticks([0, 5, 8, 13])
    elif time_window == 'long':
        ax.set_xlim([0, 90])
        ax.set_xticks([0, 13, 85])

    if color_by == 'cycle':
        if  Pulse_direction == 'dischrg':
            f.legend(loc='center left', bbox_to_anchor=(1, 0.5), title="Cycle", markerscale=6)
        elif Pulse_direction == 'chrg':
            f.legend(loc='upper center', bbox_to_anchor=(0.5, 0), title="Cycle",
                    ncol=5, markerscale=6)
...
f = plot_degr_pulses(color_by = 'cycle', time_window='short')
save_filename = 'Exp2_pulses_tof_short'
utils.save_figure(f, visualistion_path, save_filename, 'png')

f = plot_degr_pulses(color_by = 'cycle', time_window='short',
                     y_quant='temperature')
save_filename = 'Exp2_pulses_temp_short'
utils.save_figure(f, visualistion_path, save_filename, 'png')

f = plot_degr_pulses(color_by = 'cycle', time_window='long')
save_filename = 'Exp2_pulses_tof_long'
utils.save_figure(f, visualistion_path, save_filename, 'png')

f = plot_degr_pulses(color_by = 'cycle', time_window='long',
                     y_quant='temperature')
save_filename = 'Exp2_pulses_temp_long'
utils.save_figure(f, visualistion_path, save_filename, 'png')

# f = plot_degr_pulses(color_by = 'cycle', time_window='short', Pulse_direction='chrg')
# f = plot_degr_pulses(color_by = 'cycle', time_window='short', Pulse_direction='chrg',
#                      y_quant='temperature')
# f = plot_degr_pulses(color_by = 'cycle', time_window='long', Pulse_direction='chrg')
# f = plot_degr_pulses(color_by = 'cycle', time_window='long', Pulse_direction='chrg',
#                      y_quant='temperature')
...
def pairwise_plot(y_quant = 'cycle', standardise=True, Pulse_direction = 'dischrg',
    ...
    for Pulse_id in pulse_range:
        ...
        ax.set_ylabel('Pulse {}\n(~{} %,\n~{:.2f}V)'.format(Pulse_id+1, socs[Pulse_id], ocvs_pre_mean[Pulse_id]),
                      rotation=0, labelpad=30, va='center', fontsize=8)
...
save_filename = 'Exp2_pairwise'
utils.save_figure(f, visualistion_path, save_filename, 'png')

f, stats = pairwise_plot(y_quant = 'cycle', standardise=True)
save_filename = 'Exp2_pairwise_standardised'
utils.save_figure(f, visualistion_path, save_filename, 'png')
...
pulse_range=np.arange(15)
pulse_colorsheme = ListedColormap(cm.BuPu(np.linspace(0.2, 1, 256)))
norm = mpl.colors.Normalize(vmin = 0, vmax = pulse_range.max())

marker = 'o'
marker_s = 20
marker_c = pulse_colorsheme(norm(pulse_range))
...
handles = []
for Pulse_id in pulse_range:
    handles.append(mlines.Line2D([], [], marker='o', linestyle='None', color=marker_c[Pulse_id], markersize = 10,
            label='Pulse {}\n(~{}%, ~{:.2f}V)'.format(Pulse_id+1, socs[Pulse_id], ocvs_pre_mean[Pulse_id])))
f.legend(handles=handles, loc='center left', title='Exp 2',
...
# Save some data
import json
with open(os.path.join(ancillary_data_path,'stats_exp2.json'), 'w') as fp:
    json.dump(stats, fp)
